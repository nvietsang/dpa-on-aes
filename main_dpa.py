...
for bytej in range(B16):
	...
	for g in range(B256):
		...
		accu0 = np.array([0]*T)
		accu1 = np.array([0]*T)
		print('Byte: ', bytej, ' - Guess value: ',g,' - Key: ',key)
		for i, c in enumerate(ciphertext): # 2000
			vij = apply_inv_sbox(c[bytej]^g)
			# lsb = vij & 1 # 2nd
			lsb =  vij & 2
			if lsb == 2:
				accu0 = accu0 + trace[i]
		...
		delta_g = np.abs(accu1/count1 - accu0/count0)
		delta.append(np.max(delta_g))

	delta = np.array(delta)
	assert delta.size == B256
	keyj = delta.argmax()
	key.append(keyj)
